...
local BUTTON_HGT = FONT_HGT_SMALL + 4
...
function ISEquippedItem:prerender()
    ...
    if self.toDoListButton then
        local isAdmin = self.chr:getAccessLevel() ~= "" and self.chr:getAccessLevel() ~= "None"
        if isAdmin then
            if not self.toDoListButton.movedForAdmin then
                self.toDoListButton:setY(self.toDoListButton:getY() + self.adminIconOn:getHeightOrig() + 10)
                self.toDoListButton.movedForAdmin = true
            end
        else
            if self.toDoListButton.movedForAdmin then
                self.toDoListButton:setY(self.toDoListButton:getY() - self.adminIconOn:getHeightOrig() - 10)
                self.toDoListButton.movedForAdmin = false
            end
        end
...
function ISEquippedItem:initialise()
    ...
    if self.chr:getPlayerNum() == 0 then
        ...
        local width = 40
        local height = 40
        local y = self.height + self.y

        if isClient() then
            local isAdmin = self.chr:getAccessLevel() ~= "" and self.chr:getAccessLevel() ~= "None"
            if not isAdmin then
                y = y - self.adminIconOn:getHeightOrig() - 10
            end
            local safety = getServerOptions():getBoolean("SafetySystem")
            if safety then
                y = y + getTexture("media/ui/pvpicon_off.png"):getHeightOrig() + 10
            end
        end
        ...
        y = y + self.toDoListIconOff:getHeight() + 10
        self:setHeight(y)
...
ToDoListDecorations.ISSimpleTickBox.createChildren = ISSimpleTickBox.createChildren
function ISSimpleTickBox:createChildren()
    self.tickBox = ISTickBox:new(0, 0, BUTTON_HGT, BUTTON_HGT);
...
-- Preventing safety UI from rendering an invisible button if safety is off --

-- function ISSafetyUI:addToUIManager()

    -- local safety = getServerOptions():getBoolean("SafetySystem")
    -- if safety and isClient() then
