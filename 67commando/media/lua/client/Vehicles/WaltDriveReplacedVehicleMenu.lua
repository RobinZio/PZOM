...
function WWVehicleMenu.UI.addOptions(playerObj)
	...
	local seat = vehicle:getSeat(playerObj)
			...
			if (vehicle:getScriptName() == "Base.86bounder"
		or vehicle:getScriptName() == "Base.86econolinerv"
		or vehicle:getScriptName() == "Base.67commandoT50") then

		if vehicle:getPartById("Fridge") then
			if vehicle:getPartById("Fridge"):getModData().coolerActive then
				menu:addSlice(getText("ContextMenu_VehicleCoolerOff"), getTexture("media/textures/sfvehicle_cooleron.png"), WaltFunctions.onToggleCooler, playerObj)
			else
				menu:addSlice(getText("ContextMenu_VehicleCoolerOn"), getTexture("media/textures/sfvehicle_cooleroff.png"), WaltFunctions.onToggleCooler, playerObj)
			end
		end
		if vehicle:getPartById("Oven") then
			if vehicle:getPartById("Oven"):getModData().ovenActive then
				menu:addSlice(getText("ContextMenu_VehicleOvenOff"), getTexture("media/textures/sfvehicle_ovenoff.png"), WaltFunctions.onToggleOven, playerObj)
			else
				menu:addSlice(getText("ContextMenu_VehicleOvenOn"), getTexture("media/textures/sfvehicle_ovenon.png"), WaltFunctions.onToggleOven, playerObj)
			end
		end
