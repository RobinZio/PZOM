...
function Vehicles.ContainerAccess.WaltOvenAlways(vehicle, part, seat)
    return true
end

function Vehicles.Update.Fridge(vehicle, part, elapsedMinutes)
	print(part:getItemContainer():getTemprature())
...
function Vehicles.Use.Oven(vehicle, cont, player)
	local id = vehicle:getId()
	if isClient() then
		sendClientCommand(getPlayer(), 'WaltDrive', 'useCarOven', { player = player:getUsername(), id = id })
	else
...
function Vehicles.Init.Fridge(vehicle, part, player)
	if part:getModData().coolerActive then
		part:getItemContainer():setCustomTemperature(0.2)
	else
		part:getItemContainer():setCustomTemperature(1)
...
function Vehicles.Init.Oven(vehicle, part, player)
	if part:getModData().coolerActive then
